...
library BabyJubjubSig {
    ...
    function verify(bytes32 hash, bytes calldata sig, address owner)
        internal
        pure
        returns (bool)
    {
        ...
        assembly {
            r8x := calldataload(sig.offset)
            r8y := calldataload(add(sig.offset, 32))
            s   := calldataload(add(sig.offset, 64))
        }
